...
void BioZone6_updater::showEvent(QShowEvent *ev)
{
...
bool BioZone6_updater::isConnectionOk()
{
...
bool BioZone6_updater::isUpdateAvailable()
{
...
void BioZone6_updater::doDownload(const QUrl & _url)
{
...
void BioZone6_updater::abortDownload()
{
...
void BioZone6_updater::cleanTempFolder()
{
...
void BioZone6_updater::downloadProgress(qint64 bytesReceived, qint64 bytesTotal)
{
...
QString BioZone6_updater::saveFileName(const QUrl & _url)
{
...
bool BioZone6_updater::saveToDisk(const QString & filename, QIODevice * data)
{
	QFile file(filename);
...
bool BioZone6_updater::isHttpRedirect(QNetworkReply * reply)
{
...
QString BioZone6_updater::getHttpRedirectTarget(QNetworkReply *reply)
{
	QString target = reply->attribute(QNetworkRequest::RedirectionTargetAttribute).toString();
	return target;
}

void BioZone6_updater::downloadFinished(QNetworkReply * reply)
{
	ui_updater->pushButton_cancelDownload->setEnabled(false);
	QUrl url = reply->url();
	if (reply->error()) {
...
void BioZone6_updater::sslErrors(const QList<QSslError> &sslErrors)
{
#if QT_CONFIG(ssl)
...
void BioZone6_updater::startUpdate()
{
	m_start_timer->stop();
...
bool BioZone6_updater::checkConnection()
{
	if (this->isConnectionOk())
	{
...
void BioZone6_updater::downloadInstaller()
{
...
bool BioZone6_updater::askPassword()
{
...
void BioZone6_updater::showDetails()
{
...
bool BioZone6_updater::read_xmlinfo_file(QString _file_path)
{
	...
	while (xml.readNextStartElement()) {
		if (xml.name() == QLatin1String("biozone")) {
			while (xml.readNextStartElement()) {
...
bool BioZone6_updater::read_downloaded_installer(QString _file_path)
{
	QFile file(_file_path);
	QFileInfo fileInfo(file.fileName());
	//QString filename(fileInfo.fileName());
	QString absolutePath = fileInfo.absolutePath();
	file.rename(QString(absolutePath + "/biozone_installer.exe"));
	...
	if (resBtn != QMessageBox::Yes) {
		...
		QMessageBox::warning(this, m_str_warning, m_str_close_biozone);
...
bool BioZone6_updater::compareVersions(QString _current_version, QString _online_version)
{
...
bool BioZone6_updater::updateValidityCheck()
{
...
void BioZone6_updater::initCustomStrings( )
{
	...
	m_str_close_biozone = tr("BioZone6 will be closed for the update");
	m_str_check_connection = tr("BioZone6 could not reach fluicell.com, check your internet connection");
...
void BioZone6_updater::switchLanguage(QString _translation_file)
{
...
void BioZone6_updater::closeEvent(QCloseEvent *_event) {

	std::cout << QDate::currentDate().toString().toStdString() << "  "
		<< QTime::currentTime().toString().toStdString() << "  "
		<< "BioZone6_updater::closeEvent   " << std::endl;
...
BioZone6_updater::~BioZone6_updater() {
