...
static const char blankString[] = QT_TRANSLATE_NOOP("BioZone_console", "N/A");

BioZone_console::BioZone_console(QWidget *parent ):
									 QDialog (parent),
                                     ui_console (new Ui::BioZone_console),
...
// initialize and connect serial port objects
  ...
  //	                         (&QSerialPort::error),
  //	      this, &BioZone_console::handleError);
  connect(serial, &QSerialPort::errorOccurred, this, &BioZone_console::handleError);
  connect(serial, &QSerialPort::readyRead, this, &BioZone_console::readData);
  ...
  connect(ui_console->comboBox_serialInfo,
	  ...
	  &BioZone_console::showPortInfo);

}

void BioZone_console::connectCOM()
{
...
void BioZone_console::disconnectCOM()
{
	if (serial->isOpen())
		serial->close();
...
void BioZone_console::emumeratePorts()
{
...
void BioZone_console::showPortInfo(int idx)
{
	if (idx == -1)
		return;
...
void BioZone_console::keyPressEvent(QKeyEvent *e)
{
    switch (e->key()) {
    ...
    default:
        if (localEchoEnabled)
			BioZone_console::keyPressEvent(e);
        emit getData(e->text().toLocal8Bit());
    }
}

void BioZone_console::mousePressEvent(QMouseEvent *e)
{
    Q_UNUSED(e)
    setFocus();
}

void BioZone_console::mouseDoubleClickEvent(QMouseEvent *e)
{
    Q_UNUSED(e)
}

void BioZone_console::contextMenuEvent(QContextMenuEvent *e)
{
    Q_UNUSED(e)
}

void BioZone_console::readData()
{
...
void BioZone_console::handleError(QSerialPort::SerialPortError error)
{
...
BioZone_console::~BioZone_console()
{
  delete ui_console;
}
